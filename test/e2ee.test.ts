...
test("e2ee generates valid unique secp256k1 keypairs", () => {
...
test("e2ee encrypts all messages with the caller-provided client session key", () => {
...
test("e2ee roundtrips multiline and unicode content", () => {
  const recipient = generateVeniceE2EEKeypair();
  const clientSession = generateVeniceE2EEKeypair();
...
test("e2ee rejects non-E2EE-looking payloads", () => {
...
test("e2ee normalizes attested public keys", () => {
